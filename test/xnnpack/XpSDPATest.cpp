...
TEST_F(XpTest, SDPAModule) {
    ...
    int H = 1;
    int D = 8;
    Tensor Q(B, S, H, D, Context::Instance().globalBackends(MLLM_XNNPACK), true);
    Q.setTtype(TensorType::INPUT_TENSOR);
    Tensor K(B, S, H, D, Context::Instance().globalBackends(MLLM_XNNPACK), true);
    K.setTtype(TensorType::INPUT_TENSOR);
    Tensor V(B, S, H, D, Context::Instance().globalBackends(MLLM_XNNPACK), true);
