...
class QwenDecoderNPUPart2_RomeEdit : public Module {
protected:
    int hidden_size;
    int num_heads;
    int head_dim;
...
public:
    QwenDecoderNPUPart2_RomeEdit() = default;
    QwenDecoderNPUPart2_RomeEdit(const QWenNPUConfig &config, const QWenNameConfig &names, int chunk_size, const string &base_name) {
        hidden_size = config.hidden_size;
        num_heads = config.num_attention_heads;
        head_dim = config.hidden_size / num_heads;
        intermediate_size = config.intermediate_size;
        ...
        edit_add = Add(mlp_base_name + "edit_add");
    }

    std::vector<Tensor> Forward(std::vector<Tensor> inputs, std::vector<std::any> args) override {
        auto atten_output = inputs[0];
        auto res = inputs[1];
        auto edit_tensor = inputs[2]; // Edit tensor
        ...
        gate_out = edit_add(gate_out, edit_tensor); // Add the edit tensor
        return {gate_out};
    }
};

class QwenDecoderNPUPart2WithShadow final : public QwenDecoderNPUPart2_RomeEdit {
public:
...
class QwenNPU_CPUDecoder_RomeEdit final : public Module {
    int hidden_size;
    int num_heads;
    int head_dim;
    int num_key_value_heads;
    ...
    unique_ptr<QwenDecoderNPUPart1> part1;
    QwenQKVmm qkv_mm;
    unique_ptr<QwenDecoderNPUPart2_RomeEdit> part2;

    // ROME edit vector, for implementation simplicity, the vector is in shape of (seq, dim) for QNN ElementwiseAdd
    RomeVector editVector;

    std::set<int> shadowLayer;

public:
    QwenNPU_CPUDecoder_RomeEdit() = default;
    QwenNPU_CPUDecoder_RomeEdit(const QWenNPUConfig &config, const QWenNameConfig &names, int chunk_size, const string &base_name) :
        shadowLayer(config.shadow_layers) {
        hidden_size = config.hidden_size;
        num_heads = config.num_attention_heads;
        head_dim = config.hidden_size / num_heads;
        ...
        part2 = make_unique<QwenDecoderNPUPart2_RomeEdit>(config, names, chunk_size, base_name);

        editVector = RomeVector(1, chunk_size, 1, config.hidden_size, base_name + "edit_vector");
    ...
    vector<Tensor> Forward(vector<Tensor> inputs, vector<std::any> args) override {
        ...
        //  get the edit tensor
        //  for implementation simplicity, the edit tensor is in shape of (seq, dim)
        auto edit_tensor = editVector();

        _SubgraphStart_2({o_x, res, edit_tensor});

        auto out_part2 = (*part2)({o_x, res, edit_tensor});
...
class QWenModel_NPU final : public Module {
    ...
    static vector<unique_ptr<Module>> ListWithRome(int n, std::set<int> &romeLayer, Args &&...args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (romeLayer.find(listIdx) != romeLayer.end()) {
...
public:
    ...
    QWenModel_NPU(const QWenNPUConfig &config, const QWenNameConfig &names, int chunk_size, const string &base_name, std::set<int> &romeLayer) {
        // blocks = List<QwenNPU_CPUDecoder>(1, config, names, base_name);
        blocks = ListWithRome<QwenNPU_CPUDecoder, QwenNPU_CPUDecoder_RomeEdit>(config.num_hidden_layers, romeLayer, config, names, chunk_size, base_name);
...
public:
    QWenForCausalLM_NPU(QWenNPUConfig &config, int chunk_size = 64) :
        ...
        if (tie_embedding_words) {
            ...
            // use Linear rather than HeadLinear for loss compute
